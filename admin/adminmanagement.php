          ...
          if($_POST['password'] !== $_POST['passwordConfirm']) {
            ...
            if($adminOldEmail !== $email && $usedEmail > 0){
              ...
              if($database->query($query)) {
                ...
                $_SESSION['msg'] = 'Admin details has been updated successfully. :)';
                if(isset($_SESSION['userType'])){
                  $sql = "INSERT INTO logging(userName, userType,activityType,logMsg)
                  VALUES('{$_SESSION["adminId"]}','Admin','Updating Admin','The admin with email {$email} succesfully updated by admin {$_SESSION["adminId"]} ')";
                  $database->query($sql);
                }
              } else {
