 ...
 <?php
   ...
   if(isset($_POST['update'])) {
     ...
     if(
       ...
       $oldPass = MD5($database->escape_value(trim($_POST['oldPass'])));
       $newPass = MD5($database->escape_value(trim($_POST['newPass'])));
       $confirmPass = MD5($database->escape_value(trim($_POST['confirmPass'])));

       $sql = "SELECT * FROM admin WHERE adminEmail ='{$adminEmail}' AND adminPassword ='{$oldPass}' LIMIT 1";
       $result = $database->query($sql);
       $rowCount = mysqli_num_rows($result);
       if($rowCount == 1){
         if($newPass === $confirmPass){
           // Store the data in the database without updating the image column
           $query = "UPDATE admin SET adminPassword ='{$newPass}' WHERE adminId = '{$id}' LIMIT 1";
           if($database->query($query)) {
               ...
               $logMsg = "The user ".$adminEmail." has reset his/her password";
               if($_SESSION['userType'] === "Admin"){
                 $sql = "INSERT INTO logging(userName, userType,activityType,logMsg)
                 VALUES('{$adminEmail}','Admin','Admin Reset password','{$logMsg}')";
                 $database->query($sql);
               }else if($_SESSION['userType'] === "Operator"){
                 $sql = "INSERT INTO logging(userName, userType,activityType,logMsg)
                 VALUES('{$adminEmail}','Operator','Operator Reset password','{$logMsg}')";
                 $database->query($sql);
               }
               session_unset();
               session_destroy();
               ...
               $_SESSION['msg'] = 'Sorry! There is an error changing your password! <br /> Please try again later. :(';
               $_SESSION['type'] = 'error';
               $logMsg = "Changing password for user ".$adminEmail." failed, try again";
           }
         }else{
           $_SESSION['msg'] = 'Invalid passowrd, password confirmation failed, it does not match. :(';
           $_SESSION['type'] = 'error';
           $logMsg = "Password confirmation for user ".$adminEmail." failed, it does not match";
         }

       }else{
         // Send an error message to the user that record was not able to be save
         $_SESSION['msg'] = 'Wrong password entered, please enter your valid password. :(';
         $_SESSION['type'] = 'error';
         $logMsg = "Wrong password for user ".$adminEmail." entered";
       }
   ...
   if(isset($logMsg) && !empty($logMsg) && isset($adminEmail) && $_SESSION['userType'] === "Admin"){
     $sql = "INSERT INTO logging(userName, userType,activityType,logMsg)
     VALUES('{$adminEmail}','Admin','Admin Reset password','{$logMsg}')";
     $database->query($sql);
   }else if(isset($logMsg) && !empty($logMsg) && isset($adminEmail) && $_SESSION['userType'] === "Operator"){
     $sql = "INSERT INTO logging(userName, userType,activityType,logMsg)
     VALUES('{$adminEmail}','Operator','Operator Reset password','{$logMsg}')";
     $database->query($sql);
   }
                 ...
                 <div class="card-body box-profile">
                   ...
                   <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                     <div class="input-group mb-3">
                       <input type="password" class="form-control" placeholder="Old password" name="oldPass" required="required">
                     ...
                     <div class="input-group mb-3">
                       <input type="password" class="form-control" placeholder="Confirm new password" name="confirmPass" required="required">
                       <div class="input-group-append">
                         <div class="input-group-text">
                           <span class="fas fa-lock"></span>
                         </div>
                     ...
                     <div class="row">
                       ...
                       <div class="offset-sm-4 col-sm-4 text-center mb-4">
                         <button type="submit" name="update" id="update" class="btn btn-success btn-block">Change Password</button>
                       </div>
                       <!-- /.col -->
                     </div>
                   </form>
