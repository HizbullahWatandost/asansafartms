...
if(isset($_POST['operatorRegister'])){
      ...
      if($password !== $passwordConfirm){
        ...
        if($usedEmail > 0){
          ...
          $password = MD5($password);
            if($admin->createOperator($fullName,$email,$mobile,$password)){
              $_SESSION['type'] = 'success';
              $_SESSION['msg'] = 'The operator have uccessfully added. :)';
              ...
              $logMsg = "Registering operator ".$email." failed, please try again";
            }
          }
        }
      }else{
  ...
  if(isset($logMsg) && !empty($logMsg) && !empty($email)){
    ...
    VALUES('{$email}','Admin','Registering an operator','{$logMsg}')";
    $database->query($sql);
  }
